...
function AddressMapPicker({
  ...
  useEffect(() => {
    if (!mapInstanceRef.current || !mapReady || !addressChanged) {
      return;
    }
...
export default function AddressesPage() {
  ...
  const handleLocationChange = (partialData: Partial<FormDataState>) => {
    setFormData((prev) => ({
      ...prev,
      ...partialData,
    }));
  ...
  const handleTextChange = (field: keyof FormDataState, value: string) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    }));
  ...
  const handleSubmit = async () => {
    const { city, province, postalCode, latitude, longitude } = formData;

    const address =
      formData.address.trim() || "Pinned Location (No Street Name)";

    if (!city.trim() || !province.trim()) {
      toast.error("Please enter at least a City and Province");
      return;
    }
      ...
      {addresses.length === 0 ? (
        ...
        <div className="space-y-4">
          {addresses.map((addr) => (
            <Card
              key={addr.id}
              className={addr.isDefault ? "border-primary/50 bg-primary/5" : ""}
            >
              <CardContent className="p-4 flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <AlertDialog>
                    <AlertDialogTrigger
                      className="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground h-8 w-8 text-destructive hover:text-destructive disabled:opacity-50 disabled:pointer-events-none"
                      disabled={deletingId === addr.id}
                    >
                      {deletingId === addr.id ? (
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleTextChange("province", e.target.value)
                    }
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleTextChange("postalCode", e.target.value)
                    }
                    placeholder="4217"
                    className="h-10"
                  />
